...
export default function RegisterPage() {
  ...
  const handleWalletConnect = (name: string) => {
    toast({ title: `Connecting ${name}`, description: `Opening ${name} connection dialog...` });
    setTimeout(() => {
      toast({ title: 'Wallet Connected', description: `${name} connected successfully!` });
      handleFinish();
    }, 1500);
  };
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-6">
          ...
          {!walletStep ? (
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2"><Label>First Name</Label><Input placeholder="James" required /></div>
                <div className="space-y-2"><Label>Last Name</Label><Input placeholder="Mwangi" required /></div>
              </div>
              <div className="space-y-2"><Label>Email</Label><Input type="email" placeholder="you@example.com" required /></div>
              <div className="space-y-2"><Label>Phone (M-Pesa)</Label><Input type="tel" placeholder="+254 7XX XXX XXX" required /></div>
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                {[
                  ...
                  <button key={w.name} onClick={() => handleWalletConnect(w.name)} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:bg-muted transition-colors text-left">
              ...
              <div className="flex gap-3">
                ...
                <Button className="flex-1" onClick={() => handleWalletConnect('Core Wallet')}>Connect & Continue</Button>
              </div>
            </div>
          )}
          ...
          {!walletStep && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button variant="outline" className="w-full" onClick={() => toast({ title: 'Google Sign-Up', description: 'Google OAuth would open here.' })}>
                ...
                <Button variant="outline" className="w-full" onClick={() => toast({ title: 'Wallet Sign-Up', description: 'Wallet connection dialog would open here.' })}>
