        ...
        <>
          ...
          <div className="sm:hidden space-y-3">
            {transactions.map((t: any) => (
              <div key={t.id} className="bg-card rounded-xl border border-border p-4 shadow-card">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    <p className="font-display text-foreground">KES {parseFloat(t.amount || 0).toLocaleString()}</p>
                ...
                <div className="flex items-center justify-between">
                  <span className="uppercase text-xs font-medium text-muted-foreground">
                    {t.method}{t.cryptoToken && <span className="ml-1 text-primary">({t.cryptoToken})</span>}
                  </span>
